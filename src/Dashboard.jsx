...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function getData() {
      const {
        data
      } = await axios.get('http://localhost:800/getDatabases')
      setDatabases(data)
    }
  ...
  return ( < >
...

    {
      databases.map(data => ( < >
        ...
        className = "databaseItem"
        onClick = {
          dataItem.bind(this, data.Database)
        }
        key = {
          data.Database
        } > {
    ...
    < /Frame> < /div > < div className = "rightPanel" >
    < div className = "nav" >
    < Frame ref = {
      navFrame
    }
    ...
    className = 'navBar' >
    < Text as = 'h2' >
    Visualizar | Estrutura | Pesquisar | Inserir | SQL | Gatilhos & Eventos &
    Functions | < /Text> < /Frame >
    <
    /div>  < Frame ref = {
    contentMain
  }
